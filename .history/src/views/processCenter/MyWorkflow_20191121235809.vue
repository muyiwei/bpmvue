...
<el-collapse accordion >
  <el-collapse-item  v-for="workflows in workFlowGroups" :key="workflows.Code">
    <template slot="title">
      {{workflows.DisplayName}}<i class="header-icon el-icon-info" @click="togge(workflows)"></i>
    </template>
    <template v-show="workflows.show"><div v-for="workflow in workflows.children" :key="workflow.Code">{{workflow.DisplayName}}</div></template>
